...
class ConversionResponse(BaseModel):
    ...
    from_currency: str = Field(..., alias="from", description="Source currency code (e.g., EUR)")
    to_currency: str = Field(..., alias="to", description="Target currency code (e.g., TRY)")
    rate: float = Field(..., description="The actual exchange rate used")
    result: float = Field(..., description="The final converted amount")
    rate_date: str = Field(..., description="The actual date the rate belongs to (published by ECB)")
...
@app.get(
    ...
    responses={
        400: {"model": ErrorResponse, "description": "Validation Error (e.g., future date, missing amount)"},
        502: {"model": ErrorResponse, "description": "Bad Gateway (Upstream API down or returning invalid data)"}
    },
...
async def convert(
    amount: str = Query(None, description="The amount to convert (must be strictly positive, max 4 decimal places)", example="250"),
    from_currency: str = Query(None, alias="from", description="Source currency code", example="EUR", max_length=3),
    to_currency: str = Query(None, alias="to", description="Target currency code", example="TRY", max_length=3),
    target_date: str = Query(None, alias="date", description="Date in YYYY-MM-DD format", example="2026-08-28")
):
    if not amount:
