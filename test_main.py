...
def test_too_many_decimals():
    response = client.get(
        "/tools/convert?amount=10.12345&from=EUR&to=TRY&date=2026-08-28"
    )
    assert response.status_code == 400
...
def test_future_date():
    ...
    response = client.get(
        f"/tools/convert?amount=10&from=EUR&to=TRY&date={future_date}"
    )
    assert response.status_code == 400
...
def test_upstream_error(httpx_mock):
    httpx_mock.add_response(
        url="http://localhost:1/v1/2026-08-28?base=EUR&symbols=TRY", status_code=500
    )
